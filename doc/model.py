...
class DenseClip(nn.Module):
    def __init__(self, name: str, classnames: List[str] = None, templates: List[str] = None,
        ...
        # 3. 加载 AnyUp / TTAUP 引导上采样模块
        print(f"正在加载上采样模块...")
        try:
            self.any_up = torch.hub.load("wimmerth/anyup", "anyup", verbose=False).to(self.device).eval()
        except Exception as e:
            print(f"警告：AnyUp 加载失败({e})，将尝试使用 TTAUP 或线性插值。")
            self.any_up = None
        
        # 这里的 TTAUP 假设是一个 nn.Module 类
        self.TTAUP = TTAUP # 如果有具体的 TTAUP 实现类，请在此初始化
    ...
    def _stem(self, x, hr_guide: Optional[torch.Tensor] = None):
        ...
        # 将特征还原为 2D 形状
        lr_features = debiased_patches.permute(0, 2, 1).reshape(B, self.feat_dim, grid_h, grid_w)

        # --- 5. 引导上采样 ---
        guide = hr_guide if hr_guide is not None else x
        if self.TTAUP is not None:
            up_features = self.TTAUP(guide, lr_features)
