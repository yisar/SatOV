...
def apply_dense_crf(img_np, probs_np):
    ...
    d.addPairwiseBilateral(sxy=(40, 40), srgb=(13, 13, 13), rgbim=img_np, 
...
def main():
    ...
    # 类别定义与配色
    classnames = ['pool', 'grass', 'pavement', 'road', 'tree', 'river', 'cropland', 'building', 'house']
    custom_palette = [
    ...
    with Image.open(args.filename, 'r').convert('RGB') as raw_image:
        ...
        for y in y_steps:
            for x in x_steps:
                ...
                input_clip = transforms.Compose([
                    transforms.Resize((win, win)), transforms.ToTensor(), clip_norm
                ])(crop).unsqueeze(0).to(args.device)
                
                input_guide = transforms.Compose([
                    transforms.Resize((win*2, win*2)), transforms.ToTensor(),
        ...
        ax[1].set_title("Sliding Window + CRF Refined")
        ax[1].axis('off')
