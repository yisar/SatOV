...
# from your_model_file import ClearCLIPFeature  # ← 改成你的路径
# =========================
...
# =========================
# preprocess（和 CLIP 对齐）
# =========================
def get_transform(size=384):
    return T.Compose([
        T.Resize((size, size)),
        T.ToTensor(),
        T.Normalize(
...
def pca_vis(feat):
    ...
    feat: [1, C, H, W]
    """
    feat = feat[0].permute(1,2,0).reshape(-1, feat.shape[1]).cpu().numpy()

    pca = PCA(n_components=3)
    feat = pca.fit_transform(feat)

    feat = feat.reshape(1, -1, 3)
    feat = (feat - feat.min()) / (feat.max() - feat.min() + 1e-6)

    return feat.reshape(int(np.sqrt(feat.shape[1])), -1, 3)
...
def heatmap(feat):
    ...
    """
    hm = feat.norm(dim=1)[0].cpu().numpy()
...
def visualize(img_path, model_path=None):
    ...
    model = ClearCLIPFeature().to(device).eval()
    ...
    # original image
    # -------------------------
    img_vis = denorm(x[0]).permute(1,2,0).numpy()

    # -------------------------
    # PCA feature
    # -------------------------
    pca_img = pca_vis(feat)
    ...
    plt.subplot(1,3,3)
    plt.title("Feature Norm Heatmap")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img", type=str, default="asset/parrot.png")
    parser.add_argument("--ckpt", type=str, default=None)

    args = parser.parse_args()

    visualize(args.img, args.ckpt)